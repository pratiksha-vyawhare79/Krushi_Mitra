...
const MENU_ITEMS = [
  ...
  { id: 'crop-calendar', label: 'Crop Calendar', icon: <FiCalendar /> },
  { id: 'profit-risk', label: 'Profit & Risk', icon: <FiDollarSign /> },
  { id: 'crop-health', label: 'Crop Health', icon: <FiActivity /> },
  { id: 'notifications', label: 'Notifications', icon: <FiBell /> },
  { id: 'nearby-services', label: 'Nearby Services', icon: <FiMapPin /> },
  { id: 'voice-assistant', label: 'Voice Assistant', icon: <FiMic /> },
  ...
  return (
    <aside className="w-64 bg-white min-h-screen border-r border-gray-100 flex flex-col justify-between p-4 hidden lg:flex">
      ...
      <div className="bg-[#F8FAF5] rounded-2xl p-4 border border-green-100/60 mt-6 text-center">
        <div className="w-12 h-12 bg-white rounded-full flex items-center justify-center mx-auto mb-3 shadow-sm text-green-700 text-lg">
          <FiPhoneCall />
        </div>
        ...
        <button className="w-full bg-[#2E7D32] hover:bg-[#256427] text-white py-2.5 rounded-xl text-xs font-bold transition-all active:scale-95 shadow-sm flex items-center justify-center gap-1.5">
          <FiMic /> Start Voice Chat
        </button>
      </div>
    </aside>
  );
